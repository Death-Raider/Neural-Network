...
class NeuralNetwork extends LinearAlgebra{
	constructor({input_nodes,layer_count,output_nodes,weight_bias_initilization_range=[-0.001,0.001] } = {}){
		...
		this.cost = 0
		this.Activation = {
			hidden:[(x)=>x.map((e,i)=>(e>0)?e:e*0.1),(x,i)=>(x[i]>0)?1:0.1],
			output:[(x)=>x.map((e,i)=>1/(1+Math.exp(-e))),(x,y,i)=>x[i]*(1-x[i])]
		}
		this.loss_func = {
				out: (X,Y,cost = 0)=>{
				...
				derivative: (output_node,desired,index)=>{
					return (output_node[index]-desired[index])*this.Activation.output[1](output_node,desired,index)
				}
		}
	...
	GetLayerValues(InputVector,Activation = [this.Activation.hidden[0],this.Activation.output[0]]){//Forword Pass
		let activated = [];
		...
		for(let i = 0; i < this.Weights.length; i++){
			...
			for(let j = 0,sum = 0; j < this.Weights[i].length; sum = 0,j++){
				...
				activated[i+1].push( (sum+this.Bias[i][j]) );
			}
			activated[i+1] = Activation[(i == this.Weights.length-1)?1:0](activated[i+1])
		}
		return activated;
	}
	changes(Desired,Output){//backword pass
		let cost = this.loss_func.out(Output,Desired)
		this.cost = cost
		for(let i = 0; i < this.Nodes[this.HiddenLayerCount.length + 1].length; i++){
			this.BiasUpdates[this.Weights.length-1][i] = this.loss_func.derivative(this.Nodes[this.HiddenLayerCount.length+1], Desired, i) // output node of model and desired nodes
		...
		for(let j = this.Weights.length - 2; j > -1; j--){//iterates of all layers except the last one
			for(let k = 0,sum = 0; k < this.Weights[j].length; k++,sum = 0){
				...
				this.BiasUpdates[j][k]= (sum*(this.Activation.hidden[1](this.Nodes[j+1],k)))/((this.Nodes[j+1][k]==0)?1:this.Nodes[j+1][k]);
				for(let p = 0; p < this.Weights[j][k].length; p++) this.WeightUpdates[j][k][p] = this.BiasUpdates[j][k] * this.Nodes[j][p];
			}
		}
		return {updatedWeights:this.WeightUpdates,updatedBias:this.BiasUpdates,Cost:this.cost};
	}
	...
	update(secondTensor=this.WeightUpdates,secondMatrixBias=this.BiasUpdates,rate){//Readjustment of weights and bias
	...
	train({TotalTrain=0,trainFunc=()=>{},TotalVal=0,validationFunc=()=>{},learning_rate=0.0005,batch_train=1,batch_val=1,momentum=0}={}){
		...
		for(let i = 0; i < Math.floor((TotalTrain/batch_train+TotalVal/batch_val)); i++){
			...
			for(let b = 0; b < batch ; b++){
				let [input,desired] = (i <= Math.floor(TotalTrain/batch_train))?trainFunc(b,i):validationFunc(b,i);
				this.Nodes=this.GetLayerValues(input,[this.Activation.hidden[0],this.Activation.output[0]]);
				changing[b] = this.changes(desired,this.Nodes[this.Nodes.length-1]);
	...
	trainIteration({input,desired}={}){
		this.Nodes = this.GetLayerValues(input);
		let changing = this.changes(desired,this.Nodes[this.Nodes.length-1]);
...
class Convolution extends LinearAlgebra{
	...
	saveFilters(folder){
		...
		fs.writeFileSync(`${folder}/Filter.txt`,JSON.stringify(data));
	}
}
class MaxPool extends LinearAlgebra{
	...
	pool(inp,size = 2,stride=2,reshape=true){
		let M = inp.map((m,index)=>{
			if(m.length%stride != 0 || m[0].length%stride != 0 || size > m.length || size > m[0].length){
				throw "pooling cant be done with set stride"
			};
	...
	savePool(folder){
		...
		fs.writeFileSync(`${folder}/Pool.txt`,JSON.stringify(data));
	}
}
