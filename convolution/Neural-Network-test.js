...
class ConvolutionNeuralNetwork {
  ...
  //func for CNN
  convolutionLayers({matrix,kernal,featureMaps,stride,padding=0,type,activation} = {}){
    let conv1 = []
  ...
  forwordPass({image,layer_nodes,output_nodes,desired_outputs,learning_rate}){
    ...
    [this.FullyConnected,this.FCshape] = this.flattenFeatureMaps((this.network[this.network.length - 1][0] === "max_pool")?this.ConvLayers.Layer_4[0]:this.ConvLayers.Layer_4)
...
try{
  let x = new ConvolutionNeuralNetwork({
    conv_layers :  [["conv","linear",3],["conv","linear",3],["conv","linear",3],["conv","linear",3]],
    feature_maps : [          1      ,         1       ,           1     ,          1         ],
    strides :      [    {x:1,y:1}     ,    {x:1,y:1}    ,      {x:1,y:1}    ,   {x:1,y:1} ]
  })
  let dnn = []
  let image = x.createMatrix(10,10,1)
  ...
  for(let k = 0; k < 200000; k++){
    dnn[k] = x.forwordPass({
      ...
      layer_nodes : [],
      output_nodes : 2,
      desired_outputs : new Array(2).fill(0),
      learning_rate : 0.03
    })
    x.backwordPass()
  }
